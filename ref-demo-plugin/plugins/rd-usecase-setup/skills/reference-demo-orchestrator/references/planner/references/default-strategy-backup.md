...
Execution guard for this personalization journey:
- If Step 5 prerequisites in `auth-setup` are `No / Not sure`, mark the journey as paused/blocked and stop plan execution until user reconfirms completion.
- Do not run export/activity creation steps while this guard is unresolved.
...
| EDS UE site creation / selection | `create-eds-site` | `site-ops` |
| Repo/template/commit/push, aem-code-sync install/attach, preview check | `git-operations` | `site-ops` |
